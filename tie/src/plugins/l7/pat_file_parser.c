...
char* get_protocol_name(const char* line)
{
	...
	ptrdiff_t i = 0;
	for ( ; i < line_len; ++i) {
		if (!isspace(line[ i ])) name[i] = line[i];
		else break;
	}
	name[i] = '\0';
	return name;
		...
		if (isspace(line[ i ]) || (i == line_len - 1)) {
			...
			if (strcmp(flag, "REG_EXTENDED") == 0)
				...
				*cflags |= REG_NOSUB;
				...
				PRINTD("WARNING: encountered unknown flag %s in pattern file\n", flag);
				break;
			}
			flag[0] = '\0';
			j = -1;
...
int parse_pattern_file(const char* filename, char** pattern, int* cflags, int* eflags)
{
	...
	while ((actual_line_len = getline(&line, &line_len, pat_file) != -1)) {
		...
		if (state == PROTOCOL) {
			...
			if (strcmp(name, basename) != 0) {
				PRINTD("WARNING: Protocol declared in file does not match file name.\n"
		...
		if (state == USERSPACE) {
			if (!strchr(line, '=')) {
				PRINTD("WARNING: ignored bad line in pattern file:\n\t%s\n", line);
				continue;
			}
			...
			if (strcmp(attrib, "userspace pattern") == 0) {
				*pattern = value(line);
				...
				PRINTD("WARNING: ignored unknown pattern file attribute \"%s\"\n", attrib);
				free(attrib);
			}
		}
	}
