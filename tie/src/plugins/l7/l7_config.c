...
static int add_pattern_from_file(const char* proto_name, const char* path, int app_id, int app_sub_id);
...
int handle_config_line(const char* line, unsigned* proto_num)
{
	...
	read_options = sscanf(equal, "= %s %d %d", option_value, &app_id, &app_sub_id);
	if (read_options == 3) {
		sprintf(path, "%s/plugins/%s/%s", tie_path, plugin_name, PATTERNS_DIR);
		pat_file = find_pattern_file(path, option_value);
		ret = add_pattern_from_file(option_value, pat_file, app_id, app_sub_id);
		++(*proto_num);
		free(pat_file);
		...
		*/
			...
			if (l7_config.payload_dump != NULL) {
				 ...
				 */
				snprintf(file, MAX_BUFFER, "%s/%s", pv.directory, option_value);
...
char* find_pattern_file(const char* path, const char* proto_name)
{
	...
	if (i < 0) {
		...
		while (i--) {
			...
			snprintf(fulldirname, MAX_FN_LEN, "%s/%s", path, namelist[i]->d_name);
			...
			if ((scratchdir = opendir(fulldirname)) != NULL) {
				...
				if (!strcmp(namelist[i]->d_name, ".") || !strcmp(namelist[i]->d_name, ".."))
					...
					subdirs[d] = (char *) malloc(strlen(namelist[i]->d_name) + 1);
	...
	while (subdirs[n] != NULL) {
		int c = snprintf(filename, MAX_FN_LEN, "%s/%s/%s.pat", path, subdirs[n], proto_name);
...
int add_pattern_from_file(const char* proto_name, const char* path, int app_id, int app_sub_id)
{
	...
	ok = add_pattern(proto_name, pattern, eflags, cflags, app_id, app_sub_id);
	free(pattern);
	free(basename);
	return ok;
}
...
int l7_load_config(char* error)
{
	...
	sprintf(path, "%s/plugins/%s/%s", tie_path, plugin_name, CONFIG_FILE);
	...
	while ((read = getline(&line, &line_len, config) != -1)) {
		if (skip_line(line, read)) continue;
